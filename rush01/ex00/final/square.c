...
/*   Updated: 2018/07/15 16:18:26 by ariperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		myft_remove_line(int *GrilleSudoku, int Column)
{
	...
	while (CountC < 9)
	{
		...
		if (GrilleSudoku[Column] / (TenPowerNb ) % 10 == GrilleSudoku[CountC])
...
int     myft_remove_square(int **GrilleSudoku, int Line, int Column)
{
	int TenPowerNb;
	int CountL;
	int CountC;
	...
	while (Line == CountL - (CountL % 3))
	{
		...
		while (Column == CountC - (CountC % 3))
		{
			...
			{
				TenPowerNb = myft_10power(GrilleSudoku[CountL][CountC]);
			}
			if (GrilleSudoku[Line][Column] / (TenPowerNb) % 10 == GrilleSudoku[CountL][CountC])
